...
export class ValidationPipe implements PipeTransform<any> {
  getCodes(validationErrors: ValidationError[]) {
    return validationErrors
      .reduce((prev, current) => {
        const errorKey = current.constraints ? Object.values(current.constraints) : this.getCodes(current.children)
  ...
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    if (errors.length > 0) {
      ...
      throw new ApiException({
        type: 'validation', codes: codes.reduce((prev, curr) => [...prev, ...curr], []),
        statusCode:
